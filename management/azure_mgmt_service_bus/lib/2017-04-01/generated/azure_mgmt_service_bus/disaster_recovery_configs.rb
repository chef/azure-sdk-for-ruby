    ...
    def list_async(resource_group_name, namespace_name, custom_headers:nil)
      ...
      promise = promise.then do |result|
        ...
        if status_code == 200
          begin
            parsed_response = response_content.to_s.empty? ? nil : JSON.load(response_content)
            result_mapper = Azure::ServiceBus::Mgmt::V2017_04_01::Models::ArmDisasterRecoveryListResult.mapper()
            result.body = @client.deserialize(result_mapper, parsed_response)
          rescue Exception => e
            fail MsRest2::DeserializationError.new('Error occurred in deserializing the response', e.message, e.backtrace, result)
          end
    ...
    def create_or_update_async(resource_group_name, namespace_name, alias_parameter, parameters, custom_headers:nil)
      ...
      promise = promise.then do |result|
        ...
        if status_code == 200
          begin
            parsed_response = response_content.to_s.empty? ? nil : JSON.load(response_content)
            result_mapper = Azure::ServiceBus::Mgmt::V2017_04_01::Models::ArmDisasterRecovery.mapper()
            result.body = @client.deserialize(result_mapper, parsed_response)
          rescue Exception => e
            fail MsRest2::DeserializationError.new('Error occurred in deserializing the response', e.message, e.backtrace, result)
          end
