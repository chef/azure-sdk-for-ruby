...
module Azure::ApiManagement::Mgmt::V2018_06_01_preview
  ...
  class NetworkStatus
    ...
    def list_by_location(resource_group_name, service_name, location_name, custom_headers:nil)
      response = list_by_location_async(resource_group_name, service_name, location_name, custom_headers:custom_headers).value!
      response.body unless response.nil?
    end
    ...
    def list_by_location_with_http_info(resource_group_name, service_name, location_name, custom_headers:nil)
      list_by_location_async(resource_group_name, service_name, location_name, custom_headers:custom_headers).value!
    end
    ...
    def list_by_location_async(resource_group_name, service_name, location_name, custom_headers:nil)
      ...
      fail ArgumentError, "'location_name' should satisfy the constraint - 'MinLength': '1'" if !location_name.nil? && location_name.length < 1
      fail ArgumentError, '@client.api_version is nil' if @client.api_version.nil?
      ...
      promise = promise.then do |result|
        ...
        if status_code == 200
          begin
            parsed_response = response_content.to_s.empty? ? nil : JSON.load(response_content)
            result_mapper = Azure::ApiManagement::Mgmt::V2018_06_01_preview::Models::NetworkStatusContract.mapper()
            result.body = @client.deserialize(result_mapper, parsed_response)
          rescue Exception => e
            fail MsRest2::DeserializationError.new('Error occurred in deserializing the response', e.message, e.backtrace, result)
          end
